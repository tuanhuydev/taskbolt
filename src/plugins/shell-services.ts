...
import { SHELL_SERVICES_KEY, getShellServiceRegistry } from '@/composables/useShellServices';
...
export const shellServicesPlugin = {
  install(app: App) {
    ...
    if (registry) {
      ...
      console.log('[Shell Services] Successfully connected to shell service registry');
      ...
      if (registry.list) {
        ...
        console.log('[Shell Services] Registry info:', registry.getRegistryInfo?.());
      }
    } else {
      console.warn('[Shell Services] Shell service registry not found on window.__SHELL_SERVICES__');
    }
  },
};
